...
both model definition and compilation into an ODE function is implemented in Python.
This lowers the barrier to develop different simulation modalities,
such as perfomance optimizations by leveraging some structure in the model.
But being in Python does not mean less performance than C/C++ tools,
as the resulting ODE function can be *jitted* using numba.

These set of packages have a modular architecture with a clear separation of concerns.
As such, they can be reused, improved and extended by the broader Python community.
For example, people outside the systems biology community could
add a stochastic integrator to poincaré,
which could then be used from SimBio.
This architecture also makes them easier to understand,
reducing the difficulty to contribute fixes or new code,
assuring their maintainance and continued development into the future.
