...
where $y$ is named $\text{independent.x}$.
:::
...
Reaction system $2A \rightarrow B$ with rate $1$.
:::

::: {#fig-mass-action layout-ncol=2}
```{python}
class Model(Compartment):
    """2A -> B"""
...
Mass-Action system $2A \rightarrow B$ with rate $1 A^2$.
:::

Examples of similar reaction systems transforming $2A \rightarrow B$.
For mass-action systems, the rate is proportional to the reactants.
::::
...
and `Reaction`,
which transform reactants into products taking into account the stoichiometry (@fig-reaction).
It also includes `MassAction`, a `Reaction` subclass,
which automatically adds the reactants as part of the rate law (@fig-mass-action).
...
As poincaré and SimBio are coded in pure Python,
an expected concern is its runtime performance when integrating a model.
To test it,
we downloaded 28 models from BioModels,
and simulated them with COPASI, Tellurium/RoadRunner and simbio.
For simbio, we considered both the pure Python and numba-compiled code.
We used the LSODA solver with $10^-6$ relative and absolute tolerances,
with the exception of tellurium,
which provides the similar CVODE solver.
We measured separately the loading, first/cold run and subsequent/warm runs of each model.

For COPASI and Tellurium/RoadRunner,
we noted that its runtime depended on the number of evaluation points,
something that does not happen with SimBio (@fig-runtime, left).
Using numba to compile the right-hand-side (RHS) code generated by SimBio
results in an order of magnitude speed-up,
putting its performance on par with COPASI and RoadRunner.
If we switch the LSODA `scipy` solver for a more efficient `numbalsoda` implementation,
we get another speed-up in the SimBio runtime,
beating all other methods.
This is also true for the other models we tested (@fig-runtime, right).
