...
are a fundamental concept of modelling in numerous fields including systems biology, biochemistry, chemical engineering and epidemeology.
They are comprised of a set of chemical species or biological entities and represntation of complex interaction and transformation between them through a series reactions.
...
unless such support is specifcally developed.
...
Being ?first-ever? pure Python packages for systems modelling,
they offer extensive extensibility,
from simple tasks like reusing integrators defined in other packages,
to complex ones like altering the compilation process to leverage some structure in the equations (For example...).
...
as this is crucial for construction and simulaiton of models of whole cells and organisms,
...
Symbolite is designed to facilitate the easy integration of new backends. The source code, documentation to install and use these packages and tutorials are available at the `github page link`. 
...
2. It provides a namespace such that allows easily definition of multiple independent models in the same script,
3. It allows IDEs to provide autocomplete and refactoring capabilities (`Oscillator.<TAB>` shows `x`, `v` and `eq`),
4. It allows creatoion of instances which can be composed into a bigger model (@fig-composition).

For the latter (???, not clear rephrase, probably after the next sentence),
...
Type annotations play a significant role in static type checking, helping to identify typing errors.
For instance, `Parameter`s can be assigned either a number or a time-dependent expression.
But can also be linked to an external variable when composing models.
As such, they (what?) cannot be used to parametrize the initial conditions of variables,
for which we have to use a `Constant` (@fig-ide). (message not clear, rephrase)
...
(is `RateLaw` not better term than `MassAction` ?)
...
`Gen Comment: I amnto sure whether the number of figures are already over the limit, you may have to combine figures, while doing so, if space permits create a small schematics / summary figure, with icons and texts showing the significance / and applicability of the tools developed.`
