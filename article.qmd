...
There are many benefits of using classes to define the systems:
...
for which we have to use a `Constant` (@fig-ide).
...
As SimBio does not yet support the full SBML standard (for instance, events),
we could only simulate 60 of those.
Using the results from COPASI as ground truth,
SimBio reproduces the results with numerical differences depending on the solver tolerance.
